...
const conceptNodes = [
  { id: "binary", label: "Binary Limits", stage: "Problem", description: "Binary labels miss the range of real sleep experiences." },
  { id: "scoring", label: "Multi-Dimensional Scoring", stage: "Core Shift", description: "Moves from yes/no labeling to richer continuous scoring." },
  { id: "autoencoder", label: "Autoencoder", stage: "Techniques", description: "Learns structure from data without strict label dependency." },
  { id: "latent", label: "Latent Representation", stage: "Techniques", description: "Compresses sleep patterns into useful internal signals." },
  { id: "scaling", label: "Feature Scaling", stage: "Techniques", description: "Balances metrics so no single feature dominates." },
  { id: "bias", label: "Bias Reduction", stage: "Outcomes", description: "Less rigid dependence on simplistic labels." },
  { id: "flex", label: "Flexibility", stage: "Outcomes", description: "Adapts to varied sleep patterns across people." },
  { id: "accurate", label: "Accurate Sleep Representation", stage: "Outcomes", description: "Produces a more realistic view of sleep quality." },
];
...
const kpiMetrics = document.getElementById("kpi-metrics");
...
function buildSelectors() {
  ...
  viewSelect.add(new Option("Method Overview", "overview"));
  viewSelect.add(new Option("All Metrics (Cone)", "all"));
  viewSelect.add(new Option("Focused Metric", "metric"));
...
function stageColumn(stage) {
  ...
  const cards = nodes
    .map((n) => `<article class="node"><div>${n.label}</div><small>${n.description}</small></article>`)
    .join("");
...
function renderOverview() {
  ...
  const desc = `<div class="desc-grid">${conceptNodes.map((n) => `<article class="desc-card"><h4>${n.label}</h4><p>${n.description}</p></article>`).join("")}</div>`;
